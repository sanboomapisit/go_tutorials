...
func updateProduct(id int, product *Product) (Product, error) {
	var p Product
	// _, err := db.Exec("UPDATE public.products SET name=$1, price=$2 WHERE id=$3;",
	// 	product.Name, product.Price, id)
	row := db.QueryRow("UPDATE public.products SET name=$1, price=$2 WHERE id=$3 RETURNING id,name,price ;",
		product.Name, product.Price, id)
